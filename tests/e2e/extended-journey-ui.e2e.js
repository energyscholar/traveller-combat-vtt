 ...
 *
 * Route: Flammarion → 567-908 → Walston → Noctocol → Return
 *
...
function recordJump(from, to, parsecs) {
  ...
  metrics.fuelConsumed += parsecs * 10; // 10t per parsec for scout
...
function recordDiscovery(system, object, type) {
  metrics.discoveries.push({ system, object, type });
  console.log(`  [DISCOVERY] ${system}: ${object} (${type})`);
}
...
async function runJourneyUITest() {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
  console.log('║     EXTENDED JOURNEY UI E2E TEST (Multi-Tab Puppeteer)      ║');
  ...
  await withBrowser(async (browser, gmPage) => {
    ...
    // Check ship fuel status (if displayed)
    const fuelDisplay = await getText(gmPage, '.fuel-status, #fuel-display, .ship-fuel');
    if (fuelDisplay) {
      console.log(`  Fuel status: ${fuelDisplay}`);
    }
    test('Bridge controls visible', await waitFor(gmPage, '.alert-btn, .role-panel', 3000));

    // ==================== Phase 3: Role Panel ====================
    console.log('\n--- Phase 3: Test Role Panel ---');

    // The role panel is always present, content changes based on role
    const rolePanel = await waitFor(gmPage, '#role-panel', 2000);
    test('Role panel container found', !!rolePanel);

    // Check for role panel title
    ...
    console.log('\n--- Phase 7: Test System Map Container ---');

    // System map container exists but may be hidden by default
    const systemMapContainer = await gmPage.$('#system-map-container');
    test('System map container exists', !!systemMapContainer);

    // Check if menu button exists to open system map
...
function printUIReport() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║                    UI E2E TEST REPORT                           ║');
  console.log('╠════════════════════════════════════════════════════════════════╣');
  console.log('║  TEST COVERAGE                                                  ║');
  console.log(`║    UI Actions Performed:  ${String(metrics.uiActions).padStart(4)}                               ║`);
  ...
  if (failed > 0) {
    console.log('\n⚠️  Some UI elements may not be accessible without proper role setup.');
