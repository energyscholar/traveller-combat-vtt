...
// Traveller spacecraft - corrected to match official rules
// Scout: Type-S, 100 tons, 40 Hull, Armour 4, Thrust 2
// Free Trader: Type-A, 200 tons, 80 Hull, Armour 2, Thrust 1
const SHIPS = {
  scout: {
    name: 'Scout',
    hull: 40,      // Type-S: 100 tons = 40 Hull Points
    maxHull: 40,
    armor: 4,      // Type-S has Armour 4
    pilotSkill: 2,
    movement: 2,   // Thrust 2
    weapons: [
      {
  ...
  free_trader: {
    name: 'Free Trader',
    hull: 80,      // Type-A: 200 tons = 80 Hull Points
    maxHull: 80,
    armor: 2,      // Type-A has Armour 2
    pilotSkill: 1,
    movement: 1,   // Thrust 1
    weapons: [
      {
        ...
        damage: '3d6',
        ammo: null,
        rangeRestriction: ['adjacent', 'close', 'medium']  // Beam Lasers effective at Medium range only
      },
      {
        id: 'beamLaser',
        name: 'Beam Laser',
        damage: '3d6',
        ammo: null,
        rangeRestriction: ['adjacent', 'close', 'medium']  // Second turret with Beam Laser
      }
    ]
  }
};
...
function assignCrewRole(shipName, role, crewMember) {
  // Validate ship
  const validShips = ['scout', 'free_trader'];
...
function resolveAttack(attacker, target, options = {}) {
  ...
  if (hit) {
    ...
    const maxHull = target.maxHull || target.hull;

    // Check for critical hit (Effect ≥6 and damage > 0)
    if (triggersCriticalHit(effect, damage)) {
      ...
      result.critical.effects = applyCriticalEffects(target, location, severity, critResult.totalSeverity);
    }

    // Check for sustained damage (every 10% hull lost)
    if (triggersSustainedDamage(result.newHull, maxHull, previousHull)) {
      const location = rollCriticalLocation();
      const critResult = applyCriticalHit(target, location, 1); // Sustained damage is always Severity 1

      if (!result.critical) {
        result.critical = { triggered: false };
      }
      ...
      // Apply location-specific effects
      result.critical.sustainedEffects = applyCriticalEffects(target, location, 1, critResult.totalSeverity);
    }
  }
...
// Stage 10: Apply critical hit effects based on location
function applyCriticalEffects(target, location, severity, totalSeverity) {
  const effects = {};

  switch (location) {
    case 'mDrive':
      effects.mDrive = getMDriveEffects(totalSeverity);
      break;
    case 'powerPlant':
      effects.powerPlant = getPowerPlantEffects(totalSeverity);
      break;
    case 'sensors':
      effects.sensors = getSensorsEffects(totalSeverity);
      break;
    case 'weapon':
      effects.weapon = getWeaponEffects(severity); // Weapon uses individual severity, not total
      break;
    case 'jDrive':
      effects.jDrive = getJDriveEffects(totalSeverity);
      break;
    case 'computer':
      effects.computer = getComputerEffects(totalSeverity);
      break;
    case 'armour':
      effects.armour = getArmourEffects(totalSeverity);
      break;
    case 'hull':
      effects.hull = applyHullCritical(severity);
      // Apply immediate hull damage
      if (target.hull !== undefined) {
        target.hull = Math.max(0, target.hull - effects.hull.damage);
      }
      break;
    case 'crew':
      effects.crew = applyCrewCasualty(target, severity);
      break;
    case 'fuel':
      effects.fuel = applyFuelLeak(severity);
      break;
    case 'cargo':
      effects.cargo = { message: 'Cargo damaged (no mechanical effect)' };
      break;
    default:
      effects.unknown = { message: `Unknown location: ${location}` };
  }
...
function formatAttackResult(result) {
  ...
  if (result.hit) {
    ...
    // Stage 10: Critical hit information
    if (result.critical) {
      if (result.critical.triggered) {
        text += `\n⚠️ CRITICAL HIT! ${result.critical.message}`;
        if (result.critical.effects) {
          const effectMsg = getCriticalEffectMessage(result.critical.effects);
          if (effectMsg) {
            text += `\n   ${effectMsg}`;
          }
        }
      ...
      if (result.critical.sustainedDamage) {
        text += `\n⚠️ SUSTAINED DAMAGE! ${result.critical.sustainedDamage.message}`;
        if (result.critical.sustainedEffects) {
          const effectMsg = getCriticalEffectMessage(result.critical.sustainedEffects);
          if (effectMsg) {
            text += `\n   ${effectMsg}`;
          }
        }
...
// Stage 10: Format critical effect message for display
function getCriticalEffectMessage(effects) {
  const messages = [];

  for (const [system, effect] of Object.entries(effects)) {
