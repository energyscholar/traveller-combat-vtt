...
function register(socket, io, state) {
  ...
  socket.on('ops:assignRole', (data) => {
    try {
      ...
      // Check if this specific role instance is available
      if (!opsSession.isGuest) {
  ...
  // Sensor scan (sensor operator action)
  socket.on('ops:sensorScan', (data) => {
    try {
      ...
      // For now, just return all visible contacts - actual skill check would happen here
      const contacts = operations.getVisibleContacts(opsSession.campaignId, opsSession.shipId);
      ...
      operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
        gameDate: campaign.current_date,
        entryType: 'sensor',
        message: `${scanType === 'active' ? 'Active' : 'Passive'} sensor scan performed`,
        actor: opsSession.isGuest ? opsSession.guestName : 'Sensors'
      });
      socketLog.info(`[OPS] Sensor scan: ${scanType}`);
    } catch (error) {
  ...
  socket.on('ops:completeJump', () => {
    try {
      ...
      // Broadcast to ship crew and campaign
      io.to(`ops:bridge:${opsSession.shipId}`).emit('ops:jumpCompleted', result);
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:locationChanged', {
        newLocation: result.arrivedAt
      });
      socketLog.info(`[OPS] Jump completed, arrived at ${result.arrivedAt}`);
    } catch (error) {
