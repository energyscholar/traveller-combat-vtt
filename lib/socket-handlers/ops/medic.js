...
function register(ctx) {
  ...
  // Hamburger menu: Get medical records summary
  socket.on('ops:getMedicalRecords', () => {
    try {
      if (!opsSession.campaignId) return;
      const health = crewHealth.getCampaignHealth(opsSession.campaignId);
      const triage = crewHealth.triage(opsSession.campaignId);
      socket.emit('ops:medicalRecords', { health, triage });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Medic', error));
    }
  });
