...
function register(ctx) {
  ...
  socket.on('ops:plotJump', (data) => {
    try {
      ...
      // AR-131: Captain can also execute astrogation commands in solo mode
      if (!opsSession.isGM && opsSession.role !== 'astrogator' && opsSession.role !== 'pilot' && opsSession.role !== 'captain') {
        socket.emit('ops:error', { message: 'Only astrogator, pilot, or captain can plot jump courses' });
        return;
      }
      ...
      if (campaign.current_system && campaign.current_system.toLowerCase() === destination.toLowerCase()) {
        socket.emit('ops:jumpPlotted', {
          destination,
          distance,
          canJump: false,
          error: `Already in ${destination} system - select a different destination`
        });
        return;
      }
  ...
  socket.on('ops:initiateJump', (data) => {
    try {
      ...
      // AR-131: Captain can also initiate jump in solo mode
      if (!opsSession.isGM && opsSession.role !== 'astrogator' && opsSession.role !== 'pilot' && opsSession.role !== 'captain') {
        socket.emit('ops:error', { message: 'Only astrogator, pilot, or captain can initiate jump' });
        return;
      }
      const { destination, distance = 1 } = data;
      if (!destination) {
  ...
  socket.on('ops:completeJump', () => {
    try {
      ...
      // AR-124: Check campaign setting for position verification requirement
      const requireVerification = updatedCampaign.require_position_verification !== 0;
      const positionVerified = requireVerification ? false : true;

      // AR-124: Update ship state with position verification status
      if (shipForUpdate) {
        operations.updateShipState(opsSession.shipId, { positionVerified });
      }

      // AR-124: Include positionVerified state for client sync
      // AR-126: Include sector for jump map recentering
      io.to(`ops:bridge:${opsSession.shipId}`).emit('ops:jumpCompleted', {
        ...result,
        positionVerified,
        autoVerified: !requireVerification,  // Let client know if auto-verified
        newDate: updatedCampaign.current_date,
        hex: destSystem?.hex || systemData?.system?.hex || null, // AR-103: Include hex for bridge header
        sector: systemData?.system?.sector || updatedCampaign.current_sector || null, // AR-126: Include sector
      ...
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:locationChanged', {
        ...
        newDate: updatedCampaign.current_date,
        hex: destSystem?.hex || systemData?.system?.hex || null,
        sector: systemData?.system?.sector || updatedCampaign.current_sector || null,
        contacts: newContacts
      });
  ...
  socket.on('ops:verifyPosition', () => {
    try {
      ...
      // AR-131: Only astrogator, captain, or GM can verify position
      if (opsSession.role !== 'astrogator' && opsSession.role !== 'captain' && !opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only the Astrogator or Captain can verify position' });
        return;
      }
      ...
      // Check if position needs verification
      if (ship.current_state?.positionVerified !== false) {
        socket.emit('ops:error', { message: 'Position already verified or not needed' });
        return;
      }

      // Perform Electronics (Sensors) check - simplified: 2d6 + skill, 8+ success
      // Bonuses: +2 for good jump (simulated), +2 for good star maps (assumed)
      const roll = Math.floor(Math.random() * 6) + 1 + Math.floor(Math.random() * 6) + 1;
      const skillBonus = 2;  // Assume competent astrogator
      const jumpBonus = 2;   // Assume good jump
      const total = roll + skillBonus + jumpBonus;
      const success = total >= 8;
      const effect = total - 8;

      // Update ship state
      operations.updateShipState(opsSession.shipId, { positionVerified: true });

      // AR-110: Get current campaign data to send with verification
      const campaign = operations.getCampaign(opsSession.campaignId);

      // Broadcast result
      io.to(`ops:bridge:${opsSession.shipId}`).emit('ops:positionVerified', {
        success,
        roll,
        total,
        effect,
        // AR-110: Include location data so Astrogator panel refreshes correctly
        currentSystem: campaign?.current_system,
        currentHex: campaign?.current_hex,
        currentSector: campaign?.current_sector,
        message: success
          ? `Position verified! Effect +${effect}. Pilot may now navigate.`
          : `Position verified with difficulty (Effect ${effect}). Pilot may proceed.`
      });

      socketLog.info(`[OPS] Position verified: roll=${roll}, total=${total}, success=${success}`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Position', error));
      socketLog.error('[OPS] Error verifying position:', error);
    }
  });
}
