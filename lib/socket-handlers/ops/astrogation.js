...
function register(ctx) {
  ...
  socket.on('ops:verifyPosition', () => {
    try {
      ...
      // Only astrogator or GM can verify position
      if (opsSession.role !== 'astrogator' && !opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only the Astrogator can verify position' });
        return;
      }
