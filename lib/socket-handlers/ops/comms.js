...
function register(ctx) {
  ...
  // --- Real-time Comms System (AR-39) ---

  // Get comms state
  socket.on('comms:getState', () => {
    try {
      if (!opsSession.campaignId) return;
      const state = operations.getCommsState(opsSession.campaignId);
      const channels = operations.getChannels(opsSession.campaignId);
      const signal = operations.getEffectiveSignal(opsSession.campaignId);
      socket.emit('comms:state', { state, channels, signal });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });

  // Get recent transmissions
  socket.on('comms:getTransmissions', (data = {}) => {
    try {
      if (!opsSession.campaignId) return;
      const { limit = 50, channelId } = data;
      const transmissions = channelId
        ? operations.getTransmissionsByChannel(channelId, limit)
        : operations.getRecentTransmissions(opsSession.campaignId, limit);
      socket.emit('comms:transmissions', { transmissions });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });

  // Send transmission (Comms role)
  socket.on('comms:sendTransmission', (data) => {
    try {
      if (!opsSession.campaignId) return;
      const transmission = operations.sendTransmission(opsSession.campaignId, {
        ...data,
        type: 'outgoing',
        fromName: opsSession.playerName || 'Bridge'
      });

      // Broadcast to all bridge crew
      io.to(`campaign:${opsSession.campaignId}`).emit('comms:newTransmission', transmission);
      socket.emit('comms:sent', { transmission });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });
  ...
  socket.on('comms:toggleChannel', (data) => {
    try {
      const { channelId, monitoring } = data;
      const channel = operations.toggleChannelMonitoring(channelId, monitoring);
      socket.emit('comms:channelUpdated', { channel });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });
  ...
  // GM: Create channel
  socket.on('comms:createChannel', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can create channels' });
        return;
      }
      const channel = operations.createChannel(opsSession.campaignId, data);
      io.to(`campaign:${opsSession.campaignId}`).emit('comms:channelCreated', { channel });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });
  ...
  // GM: Set jamming
  socket.on('comms:setJamming', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can set jamming' });
        return;
      }
      const { jamming, intensity = 50 } = data;
      const state = operations.setJamming(opsSession.campaignId, jamming, intensity);
      io.to(`campaign:${opsSession.campaignId}`).emit('comms:stateUpdated', {
        state,
        signal: operations.getEffectiveSignal(opsSession.campaignId)
      });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  ...
  // GM: Trigger incoming transmission (for adventures)
  socket.on('comms:incomingTransmission', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can trigger transmissions' });
        return;
      }
      const transmission = operations.sendTransmission(opsSession.campaignId, {
        ...data,
      ...
      // Broadcast to all - this is an incoming transmission
      io.to(`campaign:${opsSession.campaignId}`).emit('comms:newTransmission', transmission);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  ...
  // Mark transmission read
  socket.on('comms:markRead', (data) => {
    try {
      const { transmissionId } = data;
      operations.markTransmissionRead(transmissionId);
      socket.emit('comms:markedRead', { transmissionId });
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Comms', error));
    }
  });
