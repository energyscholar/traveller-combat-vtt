...
function register(ctx) {
  ...
  // AR-241: Create solo explorer campaign
  socket.on('ops:createSoloCampaign', (data) => {
    try {
      const rateCheck = checkRateLimit(socket, 'sensitive');
      if (!rateCheck.allowed) {
        socket.emit('ops:error', { message: rateCheck.error });
      ...
      const { campaignType, shipType, startSystem, explorerName } = data || {};

      // Create campaign with solo explorer type
      const name = `Solo Explorer - ${new Date().toLocaleDateString()}`;
      const campaign = operations.createCampaign(name, explorerName || 'Explorer');

      // Update campaign type to solo_explorer
      operations.updateCampaign(campaign.id, {
        campaign_type: campaignType || 'solo_explorer'
      });

      // Create Type S Scout as party ship
      const scoutShip = operations.addShip(campaign.id, {
        name: 'Scout Ship',
        template: shipType || 'scout',
        isParty: true
      });

      // Set starting system (Regina by default)
      operations.updateCampaign(campaign.id, {
        current_system: startSystem || 'Regina',
        current_sector: 'Spinward Marches',
        current_hex: '1910'  // Regina's hex
      });

      // Set session state
      opsSession.campaignId = campaign.id;
      opsSession.isGM = true;  // Solo player has full control
      opsSession.isSolo = true;
      socket.join(`ops:campaign:${campaign.id}`);

      // Get full campaign data and send to client
      const fullData = operations.getFullCampaignData(campaign.id);
      socket.emit('ops:campaignData', fullData);
      socket.emit('ops:soloCampaignCreated', {
        campaign,
        ship: scoutShip,
        isSolo: true
      });

      socketLog.info(`[OPS] Solo campaign created: ${campaign.id} with Scout ship`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Solo Campaign', error));
      socketLog.error('[OPS] Error creating solo campaign:', error);
    }
  });
