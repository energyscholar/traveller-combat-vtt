  ...
  socket.on('ops:fireWeapon', (data) => {
    try {
      ...
      if (ship?.ship_data?.combat?.fireControl) {
        ...
        // Fallback: check software array for Fire Control
        const fcSoftware = ship.ship_data.software.find(s =>
          s.name?.toLowerCase().includes('fire control') || s.name?.toLowerCase() === 'fire_control'
        );
        if (fcSoftware?.rating) {
          fireControlDM = fcSoftware.rating;
        }
      }
