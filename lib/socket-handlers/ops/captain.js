...
function register(ctx) {
  ...
  socket.on('ops:issueOrder', (data) => {
    try {
      ...
      const { target, order, requiresAck = true } = data || {};
      ...
      addToCommandLog(state, { type: 'order', target, order: order.trim() });

      // Broadcast to campaign
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:orderReceived', {
        id: orderId,
        from: 'Captain',
  ...
  socket.on('ops:acknowledgeOrder', (data) => {
    try {
      ...
      if (order && !order.acknowledged) {
        order.acknowledged = true;
        order.acknowledgedBy = opsSession.slotName || opsSession.role || 'Crew';
        order.acknowledgedAt = new Date().toISOString();

        // Notify Captain
        io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:orderAcknowledged', {
          orderId,
          acknowledgedBy: order.acknowledgedBy,
          timestamp: order.acknowledgedAt
        });

        socketLog.info(`[OPS] Order ${orderId} acknowledged by ${order.acknowledgedBy}`);
      }
    } catch (error) {
  ...
  socket.on('ops:setWeaponsAuth', (data) => {
    try {
      if (!opsSession.isGM && opsSession.role !== 'captain') {
        socket.emit('ops:error', { message: 'Only Captain or GM can authorize weapons' });
        return;
      }
      if (!opsSession.campaignId) {
        socket.emit('ops:error', { message: 'No campaign selected' });
      ...
      // Broadcast to campaign
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:weaponsAuthChanged', {
        mode,
  ...
  socket.on('ops:markContact', (data) => {
    try {
      ...
      // Broadcast to campaign
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:contactMarked', {
        contactId,
        marking,
  ...
  socket.on('ops:requestStatus', (data) => {
    try {
      ...
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:statusRequested', {
        target: target || 'all',
        requestedBy: 'Captain',
        timestamp: new Date().toISOString()
      });
  ...
  socket.on('ops:statusReport', (data) => {
    try {
      ...
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:statusReceived', {
  ...
  socket.on('ops:leadershipCheck', (data) => {
    try {
      ...
      // Broadcast result
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:leadershipResult', {
        roll,
        skill,
        dm,
        target,
  ...
  socket.on('ops:tacticsCheck', (data) => {
    try {
      ...
      // Broadcast result
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:tacticsResult', {
        roll,
        skill,
  ...
  socket.on('ops:useLeadershipDM', (data) => {
    try {
      ...
      if (state.leadershipDM) {
        ...
        io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:leadershipApplied', {
          dm,
