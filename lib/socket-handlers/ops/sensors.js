...
function register(ctx) {
  ...
  // AR-70: Targeted scan on specific contact with Mongoose Traveller skill rolls
  // Scan DMs: Passive -2, Active 0, Deep +2
  // Check: Electronics (Sensors) + INT, 8+ success
  socket.on('ops:scanContact', (data) => {
    try {
      ...
      const scanLevelMap = { passive: 1, active: 2, deep: 3 };
      const scanDMMap = { passive: -2, active: 0, deep: 2 };
      const targetLevel = scanLevelMap[scanType] || 2;
      const scanDM = scanDMMap[scanType] || 0;
      ...
      // AR-70: Mongoose Traveller sensor skill roll
      const dice = require('../../dice');
      const roller = new dice.DiceRoller();
      const roll = roller.roll2d6();
      const skillBonus = 2;  // Assume Electronics (Sensors) 2
      const intBonus = 1;    // Assume INT modifier +1
      const total = roll.total + skillBonus + intBonus + scanDM;
      const targetNumber = 8;
      const success = total >= targetNumber;
      const effect = total - targetNumber;

      // AR-98: Broadcast roll result to chat
      const scanLabel = scanType === 'deep' ? 'Deep Scan' : scanType === 'passive' ? 'Passive Scan' : 'Active Scan';
      broadcastRollResult(io, opsSession.campaignId, {
        roller: 'Sensors',
        action: scanLabel,
        dice: roll.dice,
        total: roll.total,
        modifier: skillBonus + intBonus + scanDM,
        result: total,
        target: targetNumber,
        success,
        details: contact.name || contact.transponder || contact.type || 'Contact'
      });

      const campaign = operations.getCampaign(opsSession.campaignId);
      const contactName = contact.name || contact.transponder || contact.type || 'Contact';

      if (!success) {
        // Failed roll - no new info
        operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
          gameDate: campaign.current_date,
          entryType: 'sensor',
          message: `${scanLabel} on ${contactName}: No new data (roll ${roll.total}+${skillBonus + intBonus + scanDM}=${total} vs 8)`,
          actor: opsSession.isGuest ? opsSession.guestName : 'Sensors'
        });
        ...
        socket.emit('ops:scanContactResult', {
          ...
          total,
          effect,
          message: `Scan inconclusive (Effect ${effect})`,
          contact
        });
        return;
      }

      // Success - upgrade scan level
      ...
      if (discoveries.length === 0) {
        socket.emit('ops:scanContactResult', {
          ...
          total,
          effect,
          message: 'Already at maximum scan level for this type',
      ...
      operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
        gameDate: campaign.current_date,
        entryType: 'sensor',
        message: `${scanLabel} scan on ${contactName}: Effect +${effect}. Revealed: ${disc.discoveries.join(', ')}`,
        actor: opsSession.isGuest ? opsSession.guestName : 'Sensors'
      });
      ...
      socket.emit('ops:scanContactResult', {
        ...
        message: `${contactName}: ${disc.discoveries.join(', ')} (Effect +${effect})`
      });

      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:contactUpdated', { contact: disc.contact });

      socketLog.info(`[OPS] Targeted ${scanType} scan on ${contactId}: roll ${total}, effect ${effect}`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Contact', error));
  ...
  /**
   * AR-197: Spawn a ship emerging from jump space
   * Creates contact and broadcasts emergence alert to all bridge stations
   */
  socket.on('ops:spawnJumpEmergence', (data) => {
    try {
      if (!opsSession.isGM) {
        socket.emit('ops:error', { message: 'Only GM can spawn jump emergences' });
        return;
      }
      if (!opsSession.campaignId) {
        socket.emit('ops:error', { message: 'Not in a campaign' });
      ...
      const {
        ...
        transponder = null,
        bearing = Math.floor(Math.random() * 360),
        range_km = 2000000 + Math.floor(Math.random() * 1000000), // 2-3 million km
        disposition = 'unknown',
        captain_name = null
      } = data;

      // Create the emerged contact
      const contact = operations.addContact(opsSession.campaignId, {
        name,
        type,
        tonnage,
        ...
        bearing,
        range_km,
        range_band: 'distant', // Jump emergence is always at extreme range
        disposition,
        captain_name,
        signature: 'High', // Jump flash is highly visible
        emerged: true,
        emerged_at: Date.now(),
        is_targetable: disposition === 'hostile'
      });

      // Broadcast contact added
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:contactAdded', { contact });

      // Broadcast jump emergence alert to all bridge stations
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:jumpEmergence', {
      ...
      operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
        gameDate: campaign?.current_date,
        entryType: 'sensor',
        message: `JUMP EMERGENCE: ${contact.transponder || contact.name} detected at bearing ${bearing}°, range ${Math.round(range_km / 1000)}K km`,
        actor: 'SENSORS'
      });

      socketLog.info(`[OPS] Jump emergence: ${contact.name} (${contact.id}) at bearing ${bearing}°`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Emergence', error));
      socketLog.error('[OPS] Error spawning jump emergence:', error);
    }
  });

  /**
   * AR-197: Put contact on viewscreen (center system map on contact)
   */
  socket.on('ops:putOnViewscreen', (data) => {
    try {
      const { contactId } = data;
      ...
      // Broadcast to all bridge stations to focus on this contact
      io.to(`ops:campaign:${opsSession.campaignId}`).emit('ops:viewscreenFocus', {
        contactId,
        name: contact.name || contact.transponder,
        bearing: contact.bearing,
        range_km: contact.range_km
      });

      socketLog.info(`[OPS] Viewscreen focused on: ${contact.name} (${contactId})`);
    } catch (error) {
      socket.emit('ops:error', sanitizeError('Viewscreen', error));
      socketLog.error('[OPS] Error setting viewscreen focus:', error);
    }
  });
}
...
function clearCampaignState(campaignId) {
  const timer = trainingTargetRespawnTimers.get(campaignId);
  if (timer) {
    clearTimeout(timer);
    trainingTargetRespawnTimers.delete(campaignId);
  }
...
module.exports = { register, clearCampaignState };
