...
function register(ctx) {
  ...
  socket.on('ops:scanContact', (data) => {
    try {
      ...
      const scanLabel = scanType === 'deep' ? 'Deep' : scanType === 'passive' ? 'Passive' : 'Active';
      ...
      if (!success) {
        ...
        operations.addLogEntry(opsSession.shipId, opsSession.campaignId, {
          gameDate: campaign.current_date,
          entryType: 'sensor',
          message: `${scanLabel} scan on ${contactName}: No new data (roll ${roll.total}+${skillBonus + intBonus + scanDM}=${total} vs 8)`,
          actor: opsSession.isGuest ? opsSession.guestName : 'Sensors'
        });
