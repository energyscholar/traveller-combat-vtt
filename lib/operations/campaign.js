...
function addShip(campaignId, name, shipData, options = {}) {
  ...
  // Default location to Flammarion Highport if not specified
  if (!currentState.locationId) {
    currentState.locationId = 'loc-dock-highport';
  }
...
// ==================== Combat State Sync (Autorun 12) ====================

/**
 * Sync combat state back to operations DB
 * Called at round end and combat end
 * @param {string} campaignId - Campaign ID
 * @param {string} shipId - Ship ID (player's ship)
 * @param {Object} combatState - Current combat state
 * @returns {boolean} Success
 */
function syncCombatStateToOps(campaignId, shipId, combatState) {
  try {
    ...
    const currentState = JSON.parse(ship.current_state || '{}');

    // Update ship state with combat results
    const updates = {};

    // Sync hull damage if provided
    if (combatState.playerShip?.hull !== undefined) {
      updates.hull = combatState.playerShip.hull;
    }

    // Sync fuel consumption if provided
    if (combatState.playerShip?.fuel !== undefined) {
      updates.fuel = combatState.playerShip.fuel;
    }

    // Sync ammo if provided
    if (combatState.playerShip?.ammo !== undefined) {
      updates.ammo = combatState.playerShip.ammo;
    }

    // Sync system damage if provided
    if (combatState.playerShip?.damage) {
      updates.systemDamage = combatState.playerShip.damage;
    }

    // Only update if there are changes
    if (Object.keys(updates).length > 0) {
      const newState = { ...currentState, ...updates };
      updateShipState(shipId, newState);
      return true;
    }

    return false;
  } catch (error) {
    console.error('[Campaign] Error syncing combat state:', error);
    return false;
  }
}

/**
 * Apply combat damage to ship (Traveller rules)
 * @param {string} shipId - Ship ID
 * @param {Object} damage - Damage details { hull, systems, critical }
 * @returns {Object} Updated ship state
 */
function applyCombatDamage(shipId, damage) {
  const ship = getShip(shipId);
  if (!ship) return null;

  const currentState = JSON.parse(ship.current_state || '{}');
  const maxHull = ship.ship_data?.hull?.points || 100;

  // Apply hull damage
  if (damage.hull) {
    currentState.hull = Math.max(0, (currentState.hull || maxHull) - damage.hull);
  }

  // Apply system damage
  if (damage.systems && Array.isArray(damage.systems)) {
    currentState.systemDamage = currentState.systemDamage || {};
    for (const sys of damage.systems) {
      currentState.systemDamage[sys.name] = (currentState.systemDamage[sys.name] || 0) + sys.damage;
    }
  }

  // Record critical hits
  if (damage.critical) {
    currentState.criticalHits = currentState.criticalHits || [];
    currentState.criticalHits.push({
      ...damage.critical,
      timestamp: new Date().toISOString()
    });
  }

  updateShipState(shipId, currentState);
  return currentState;
}
  ...
  try {
    ...
    return files.map(file => {
      ...
      return {
        ...
        jump: template.drives?.jump?.rating || 0,
...
module.exports = {
  ...
  // Combat sync (Autorun 12)
  syncCombatStateToOps,
  applyCombatDamage
};
