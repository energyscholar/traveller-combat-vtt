 ...
 * - Ships: Amishi (600t Q-Ship), Kimbly (100t Scout)
...
const SHIP = {
  ...
  ship_data: {
    ...
    thrust: 3,
    jump: 3,
    fuel: { capacity: 125, current: 125, jumpRangeInternal: 2 },
    crew: {
      pilot: 3,
      astrogator: 1,
      engineer: 2,
    ...
    weapons: [
      ...
      { id: 'barbette_ion', type: 'ion_cannon', name: 'Ion Barbette', damage: 'ion', mount: 'barbette', slot: 5, range: 'Medium', operator: 'Marina (Gunner-6)', notes: 'Prize-taking weapon' },
  ...
  current_state: {
    alertStatus: 'NORMAL',
    hullPoints: 240,
    fuel: 120,  // Slightly less than full - Engineer can manage fuel
    power: 100,
    ...
    systems: {
      ...
      // Barbettes
      'Ion Barbette': { status: 'operational', health: 100 },
...
function seedDoranniaCampaign() {
  ...
  // Secondary ship: Kimbly (Scout)
  const kimblyId = generateId();
  insertShip.run(
    kimblyId,
    campaignId,
    KIMBLY.name,
    KIMBLY.template_id,
    JSON.stringify(KIMBLY.ship_data),
    1,  // visible
    1,  // party ship
    JSON.stringify(KIMBLY.current_state)
  );
  console.log(`   Created: ${KIMBLY.name} (${kimblyId})`);
  ...
  console.log('   Kimbly crew:');
  for (const npc of KIMBLY_CREW) {
    const npcId = generateId();
    insertNPC.run(npcId, kimblyId, npc.name, npc.role, npc.skill_level, npc.personality, npc.is_ai);
    console.log(`   Created NPC: ${npc.name} (${npc.role})`);
  }
