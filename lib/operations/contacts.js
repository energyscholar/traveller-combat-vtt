...
function addContact(campaignId, contactData) {
  ...
  const stmt = db.prepare(`
    INSERT INTO contacts (id, campaign_id, name, type, bearing, range_km, range_band, transponder, signature, visible_to, gm_notes, is_targetable, weapons_free, health, max_health, training_target)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
function updateContact(id, updates) {
  const allowedFields = ['name', 'type', 'bearing', 'range_km', 'transponder', 'signature', 'visible_to', 'gm_notes', 'is_targetable', 'weapons_free', 'health', 'scan_level'];
  const setFields = [];
  const values = [];
