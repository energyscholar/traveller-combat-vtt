...
const SOLO_DEMO_CAMPAIGN_ID = 'SOLODEMO';
...
function seedSoloDemoCampaign(forceReset = false) {
  ...
  if (campaignExists()) {
    if (forceReset) {
      ...
      console.log('[SeedSoloDemo] Solo demo campaign already exists, skipping seed');
...
function getSoloDemoAccount() {
  return db.prepare('SELECT * FROM player_accounts WHERE campaign_id = ?').get(SOLO_DEMO_CAMPAIGN_ID);
}
