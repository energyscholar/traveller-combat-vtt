...
/**
 * AR-300.1: Clean up legacy/duplicate solo demo campaigns
 * Removes any campaigns that match solo demo patterns but don't have the canonical ID.
 * This prevents duplicates appearing in campaign list.
 */
function cleanupLegacySoloDemos() {
  // Find legacy solo demo campaigns (by name or type, excluding canonical ID)
  const legacyCampaigns = db.prepare(`
    SELECT id, name FROM campaigns
    WHERE (name LIKE '%Solo Demo%' OR campaign_type = 'solo_demo')
    AND id != ?
  `).all(SOLO_DEMO_CAMPAIGN_ID);

  if (legacyCampaigns.length === 0) {
    return 0;
  }

  console.log(`[SeedSoloDemo] Found ${legacyCampaigns.length} legacy solo demo campaign(s) to clean up`);

  for (const legacy of legacyCampaigns) {
    console.log(`[SeedSoloDemo] Removing legacy campaign: ${legacy.id} "${legacy.name}"`);
    // Delete in correct order for foreign key constraints
    db.prepare('DELETE FROM mail WHERE campaign_id = ?').run(legacy.id);
    db.prepare('DELETE FROM npc_crew WHERE ship_id IN (SELECT id FROM ships WHERE campaign_id = ?)').run(legacy.id);
    db.prepare('DELETE FROM ships WHERE campaign_id = ?').run(legacy.id);
    db.prepare('DELETE FROM player_accounts WHERE campaign_id = ?').run(legacy.id);
    db.prepare('DELETE FROM campaigns WHERE id = ?').run(legacy.id);
  }

  console.log(`[SeedSoloDemo] Cleaned up ${legacyCampaigns.length} legacy campaign(s)`);
  return legacyCampaigns.length;
}
...
function seedSoloDemoCampaign(forceReset = false) {
  ...
  // AR-300.1: Always clean up legacy duplicates first
  cleanupLegacySoloDemos();
