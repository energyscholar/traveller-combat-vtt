...
function canInitiateJump(shipId, distance) {
  ...
  // Get ship specs
  const hullTonnage = ship.ship_data?.hull || ship.template_data?.tonnage || 100;
  const maxJumpRating = ship.ship_data?.jumpRating || ship.template_data?.jumpRating || 2;
...
function initiateJump(shipId, campaignId, destination, distance) {
  ...
  // Get current fuel
  const currentFuel = ship.current_state?.fuel ?? ship.ship_data?.fuel ?? ship.template_data?.fuel ?? 40;
  const newFuel = currentFuel - check.fuelNeeded;
...
function completeJump(shipId, campaignId) {
  ...
  updateCampaign(campaignId, { current_system: destination });

  // Clear jump state and require position verification
  updateShipState(shipId, {
    jump: {
      inJump: false,
