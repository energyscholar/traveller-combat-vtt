...
const tvadvPackage = require('./tvadv-package');
const aiNpc = require('./ai-npc');
const commsSystem = require('./comms-system');
const crewHealth = require('./crew-health');
const passengerManifest = require('./passenger-manifest');
...
const { validators } = validatorsModule;
...
module.exports = {
  ...
  // Re-export all characters functions (Autorun 9)
  ...characters,

  // Re-export all combat functions (Autorun 14)
  ...combat,

  // Re-export combat engine functions (Autorun 14)
  ...combatEngine,

  // Re-export validation functions (Stage 9.5)
  ...validation,

  // Re-export .tvadv package functions (Stage 10.1)
  ...tvadvPackage,

  // Re-export AI NPC functions (AR-41)
  ...aiNpc,

  // Re-export Comms system functions (AR-39)
  ...commsSystem,

  // Re-export Crew Health functions (AR-47)
  ...crewHealth,

  // Re-export Passenger Manifest functions (AR-46)
  ...passengerManifest,

  // Re-export Library Data functions (AR-40)
  ...libraryData,

  // Re-export validators (AR-16.5)
  validators,

  // Re-export rate limiter (AR-16.8)
  rateLimiter,
  ...
  // Combat station mapping (Autorun 12)
  // Maps operations roles to combat stations
  COMBAT_STATION_MAP: {
    pilot: 'pilot',
    astrogator: 'pilot', // Assists pilot in combat
    captain: 'command',
    engineer: 'engineering',
    sensor_operator: 'sensors',
    gunner: 'turret', // Suffixed with turret number (turret_1, turret_2, etc.)
    damage_control: 'engineering',
    marines: 'security',
    medic: 'medbay',
    steward: null, // No combat station
    cargo_master: null, // No combat station
    observer: 'observer',
    gm: 'command'
  },

  /**
   * Get combat station for a role
   * @param {string} role - Operations role
   * @param {number} turretNumber - Turret assignment for gunners
   * @returns {string|null} Combat station identifier
   */
  getCombatStation(role, turretNumber = 1) {
    const station = this.COMBAT_STATION_MAP[role];
    if (station === 'turret') {
      return `turret_${turretNumber}`;
    }
    return station;
  }
};
