...
const WEAPON_DAMAGE = {
  ...
  missile_rack: 4,     // 4d6
  particle: 6,         // 6d6
  ion: 0,              // Power drain, not hull damage
  sandcaster: 0,       // Defensive only
  laser: 3             // Generic laser (beam)
};

/**
 * Called shot targets with penalties
 */
const CALLED_SHOT_TARGETS = {
  mDrive: { penalty: -2, label: 'M-Drive' },
  jDrive: { penalty: -4, label: 'Jump Drive' },
  powerPlant: { penalty: -4, label: 'Power Plant' },
  sensors: { penalty: -2, label: 'Sensors' },
  weapon: { penalty: -2, label: 'Weapon' },
  computer: { penalty: -3, label: 'Computer' },
  fuel: { penalty: -2, label: 'Fuel Tanks' }
};
...
class CombatEngine {
  ...
  initCombat(playerFleet, enemyFleet, options = {}) {
    ...
    this.stats = {
      ...
      calledShotsAttempted: 0, calledShotsHit: 0,
  ...
  resolveAttack(attacker, defender, options = {}) {
    ...
    const calledShotDM = calledShot ? (CALLED_SHOT_TARGETS[calledShot]?.penalty || -2) : 0;

    const totalDM = fc + gunnerSkill + rangeDM + calledShotDM + evasiveDM;
    ...
    if (hit) {
      this.stats.hits++;
      if (calledShot) this.stats.calledShotsHit++;
    } else {
      this.stats.misses++;
    }
    if (calledShot) this.stats.calledShotsAttempted++;
    ...
    const result = {
      ...
      destroyed: false,
      pointDefense: null,
      modifiers: { fc, gunnerSkill, rangeDM, calledShotDM, evasiveDM }
    };
    ...
    if (hit) {
      ...
      // Calculate damage
      result.damage = this.calculateDamage(weaponType, defender, effect);
      ...
      if (calledShot && result.damage > 0) {
        result.systemDamage = this.applySystemDamage(defender, calledShot);
      }
   ...
   */
  calculateDamage(weaponType, defender, effect = 0) {
    const damageDice = WEAPON_DAMAGE[weaponType] || 2;

    if (weaponType === 'ion') {
      // Ion: 3d6 power drain + effect * 10
      const ionRoll = this.rng.rollNd6(3);
      return (ionRoll.total + effect) * 10;
    }

    const damageRoll = this.rng.rollNd6(damageDice);
    const armor = defender.armour || 0;
    return Math.max(0, damageRoll.total - armor);
  }
