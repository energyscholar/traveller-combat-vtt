...
class TUIRoleAdapter {
  ...
  getRoleActions(role) {
    const actionMap = {
      ...
      gunner: ['fire_primary', 'fire_secondary', 'fire_missiles', 'point_defense', 'called_shot'],
  ...
  formatActionResult(role, action, result) {
    ...
    switch (action) {
      ...
      case 'called_shot':
        return result.hit
          ? `${indicator} Called shot → ${GREEN}${result.system}${RESET} hit!`
          : `${indicator} Called shot: ${DIM}missed${RESET}`;
      ...
      case 'target_lock':
        return result.locked
          ? `${indicator} ${RED}LOCKED${RESET} ${result.target} (+${result.attackBonus} attack)`
