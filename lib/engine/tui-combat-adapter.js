...
class TUICombatAdapter {
  ...
  onAttackResolved(event) {
    const { hit, attacker, defender, weapon, roll, totalDM, total, damage, calledShot, modifiers } = event.data;
    ...
    // Add called shot info
    if (calledShot) {
      const target = this.formatSystemName(calledShot);
      line = `${color}${attacker.name}${RESET} called shot → ${target} ${action}!`;
      if (hit && damage > 0) {
        line += ` ${YELLOW}-${damage}${RESET}`;
      }
    }
